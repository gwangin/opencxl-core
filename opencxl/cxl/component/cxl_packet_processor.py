    ...
    async def _process_incoming_packets(self):
        ...
        while True:
            try:
                ...
                if packet.is_cxl_io():
                    print("Check the type of the packet", packet.system_header.payload_type)
                    cxl_io_packet = cast(CxlIoBasePacket, packet)
                    print("[CXL Packet Processor] : cxl_io_packet.tlp_prefix.ld_id", cxl_io_packet.tlp_prefix.ld_id)
                    cxl_io_packet.tlp_prefix.ld_id += 1
                    ...
                    print("[CXL Packet Processor] : cxl_mem_packet.tlp_prefix.ld_id", cxl_mem_packet.tlp_prefix.ld_id)
                    cxl_mem_packet.tlp_prefix.ld_id += 1
